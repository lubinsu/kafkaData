...
import com.maiya.kafka.consumer.bean.InnerBlackList;
...
public class ConsumerInBlackTask implements Runnable {
    ...
    List<InnerBlackList> memberList = null;
    Put p1 = null;
    HTable hTable = null;
    ...
    public ConsumerInBlackTask(KafkaStream stream, int threadNumber, String tableName) {
    ...
    public void run() {
        ...
        initHbase.initHbase();
        String familyName = "nbhmd";
        m_tableName = "hy_innerblacklist";
                ...
                while (it.hasNext()) {
                    ...
                    if (jsonMessage != null && jsonMessage.length() != 0) {
                        try {
                            ...
                            if (table_name.equals("hy_innerblacklist")) {
                                System.out.println("enter hy_innerblacklist ");

                                table_status = jsonObject.getString("table_status");
                                System.out.println("hy_innerblacklist status = " + table_status);

                                jsonArray = jsonObject.get("data");
                                memberList = JSON.parseArray(jsonArray + "", InnerBlackList.class);
                                ...
                                if (table_status.equals("insert")) {
                                    ...
                                    if (memberList.size() > 0) {
                                        for (int i = 0; i < memberList.size(); i++) {
                                            ...
                                            p1.addColumn(Bytes.toBytes(familyName), Bytes.toBytes("sblackcode"),
                                                    Bytes.toBytes(memberList.get(i).getsBlackCode()));
                                            p1.addColumn(Bytes.toBytes(familyName), Bytes.toBytes("sblackremark"),
                                                    Bytes.toBytes(memberList.get(i).getsBlackRemark()));
